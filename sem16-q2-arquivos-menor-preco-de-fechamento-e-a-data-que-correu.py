...
def menor_preco_de_abertura(dados):
    ordenado = sorted(dados, key = itemgetter('fechamento'))
    return round(ordenado[0]['fechamento'],2), formatar_data(ordenado[0])
...
def main():
    ...
    #Qual o menor preço de fechamento e a data que correu?
    menor_preco, data = menor_preco_de_abertura(pregao)
    
    print(f'O menor preço no fechamento foi {menor_preco:.2f} em {data}.')
