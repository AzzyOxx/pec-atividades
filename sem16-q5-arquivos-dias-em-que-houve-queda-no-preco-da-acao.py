...
def variacao_negativa(pregao, mes_choice, ano_choice):
    dias_com_queda = []
    for preg in pregao:
        if preg['mes'] == mes_choice and preg['ano'] == ano_choice:
            if preg['abertura'] > preg['fechamento']:
                dias_com_queda.insert(0,(f"{preg['dia']:0>2d}/{preg['mes']:0>2d}/{preg['ano']}", preg['abertura'], preg['fechamento'], round(float(f"{preg['abertura'] - preg['fechamento']:.2f}"),2) ))

    return dias_com_queda    
...
def main():
    ...
    dias_com_queda = variacao_negativa(pregao, mes_choice, ano_choice)
    
    
    print(f'Dias de {mes_choice}/{ano_choice} que houve queda no preço da ação:')
    for dia in dias_com_queda:
        print(dia)
