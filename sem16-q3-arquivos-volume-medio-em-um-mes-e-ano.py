...
def media_volume(dados, mes_d, ano_d):
    volume_periodo = 0
    pregoes = 0
    for preg in dados:
        if preg['mes'] == mes_d and preg['ano'] == ano_d:
            volume_periodo += preg['volume']
            pregoes += 1
...
def main():
    ...
    #3. Qual o volume médio de negociações em um mês e ano informados?
    volume_negociacoes_mes = media_volume(pregao, mes_choice, ano_choice)

    #saída
    print(f'O volume médio em {mes_choice}/{ano_choice} foi {round(volume_negociacoes_mes)}.')
